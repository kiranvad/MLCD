...
% in mtD --> Final is M0,M ; Early termination is bestMgen
...
reg0 = [1];
reg = ones(5,1)*500; %each column is one regularization vector for gamma_t
...
for r = 1:max_rep
 ...
 for i=1:length(task)
    ...
    ntr = length(task(i).y);
    [L,D(i)]=lmnn2([task(i).x task(i).xv],[task(i).y task(i).yv],'quiet',1,'validation',val,'obj',0,'ntr',ntr,'maxiter',maxiter_st);
 ...
 for i=1:length(reg0)
  for j = 1:size(reg,2)
    %% run mtLMNN
    mtD{i,j}=MTlmnn_sp(task,Kg,L,'quiet',1,'regweight',reg(:,j),'regweight0',reg0(i),'weight1',0.5,'maxiter',maxiter_mt);
  end;
end;
