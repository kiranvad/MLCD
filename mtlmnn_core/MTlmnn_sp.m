function [Det]=mtLMNN(task,Kg,L0,varargin)
%
...
global M0 M0old doingcorrection bestMgen;        
% L0=[];      
% M0=[];            
% M0old=M0;          
% bestMgen.M0 = M0;

% If L0 is not specified, set it to Covariance matrix
% if isempty(L0)
%     fprintf('L0,Initial starting point L0 not specified.\nStarting with covariance matrix.\n');
%     L0 = pca(task(1).x)';
% end
...
for i=1:length(task) 
 if isfield(task(i),'L'),L=task(i).L; else L=L0; end; %zeros(size(L0));end;
...
for iter=1:pars.maxiter 
 ...
 if iter>1,                                   
   ...
   for st=1:length(state),       
       ...
       state(st).Gradient=state(st).Gradient+2*pars.regweight(st).*vec(state(st).M);
...
if all(dobreak_1),break;end;
 ...
 for st=1:length(state)
  ...
  o=o+pars.regweight(st).*sum(sum((state(st).M).^2));
  state(st).obj(iter) = o;
  obj(iter)=obj(iter)+o;
  % fprintf(' %2.4f ',o);
  % add regularizer
...
if all(dobreak_2),break;end;
  ...
  % save file
  if(mod(iter,10)==0) save(tempname,'state','iter','obj','pars','verify');end;
end;
%keyboard;
% Output
Det.obj=obj(1:iter);
